...
//! [0]
BBMReg::BBMReg(const QUuid &uuid, QObject *parent) : QObject(parent), m_context(uuid), m_isAllowed(false), m_progress(BbmRegistrationProgress::NotStarted), m_temporaryError(false), m_statusMessage(tr("Please wait while the application connects to BBM."))
{
	connect(&m_context, SIGNAL(registrationStateUpdated(bb::platform::bbm::RegistrationState::Type)), this, SLOT(processRegistrationStatus(bb::platform::bbm::RegistrationState::Type)));
	BBMReg::registerApplication();
}

void BBMReg::registerApplication()
{
	m_progress = BbmRegistrationProgress::Started;
	processRegistrationStatus(m_context.registrationState());
}

void BBMReg::processRegistrationStatus(const RegistrationState::Type state)
{
	...
	switch (m_progress)
	{
		...
		default:
			qDebug() << "Ignoring BBM Social Platform access state=" << state << "when progress=" << m_progress;
			break;
	}
}
//![2]
...
//! [3]
void BBMReg::registrationFinished()
{
...
//        break;
	...
	qDebug() << "Finished BBM Social Platform registration, success=" << m_isAllowed << "temporaryError=" << m_temporaryError;
	emit stateChanged();
}
